...
public final class TranscriptionJob {
    ...
    let http: HTTPClient
    let baseURLOverride: String?
    /// Poll cadence for `wait` (tests shrink these; defaults match Rust/Go — 3s
    /// interval, 10min timeout).
    var interval: TimeInterval = 3
    var timeout: TimeInterval = 600

    init(handle: TranscriptionHandle, apiKey: String, http: HTTPClient, baseURLOverride: String?) {
        self.handle = handle
        self.apiKey = apiKey
        self.http = http
        self.baseURLOverride = baseURLOverride
